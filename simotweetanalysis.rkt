...
(define tweets (string->jsexpr
                (with-input-from-file "Ugandatweetssample.json" (λ () (json-lines->json-array)))))
...
(define tweet-string (single-tweet-list tweet-text))

;;; Next, we capture the text from our input port, removing capitalization, 
;;; punctuation, and then extra spaces
(define tweet-string-processed (string-normalize-spaces
		   (remove-punctuation
...
(define words (document->tokens tweet-string-processed #:sort? #t))
